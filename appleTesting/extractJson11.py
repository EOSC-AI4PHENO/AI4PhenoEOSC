...
def copy_jpg_and_json(input_dir):
    ...
    max_len = max(len(unused_images), len(unused_jsons), len(used_jsons), len(used_images))
    df = pd.DataFrame({
        'Unused Images': list(unused_images) + [''] * (max_len - len(unused_images)),
        'Unused JSONs': list(unused_jsons) + [''] * (max_len - len(unused_jsons)),
        'Used JSONs': list(used_jsons) + [''] * (max_len - len(used_jsons)),
        'Used Images': list(used_images) + [''] * (max_len - len(used_images))
    })

    stats_filename = f"{modified_input_dir}_statistics.xlsx"
    #df.to_excel(os.path.join(input_dir, stats_filename), index=False)
