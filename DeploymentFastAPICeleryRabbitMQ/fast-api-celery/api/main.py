...
from worker.tasks import (get_sunrise_sunset, is_Image_WellExposedByHisto, get_apple_automatic_rois, \
    ...
    get_linden_automatic_rois_with_indicators, get_classification_linden_with_indicators, \
    get_apple_automatic_rois_with_indicators_Detectron2)
...
# START DETECTRON2
@app.post('/AutomaticAppleSegmentationDetectron2Model/get_apple_automatic_rois_with_indicators_Detectron2', response_model=TaskTicket,
          status_code=202)
async def schedule_AutomaticAppleSegmentationDetectron2Model_get_apple_automatic_rois_with_indicators_Detectron2(
        model_input: AutomaticAppleSegmentationInput):
    """Create celery prediction task. Return task_id to client in order to retrieve result"""
    task_id = get_apple_automatic_rois_with_indicators_Detectron2.delay(model_input.imageBase64, model_input.filename,
                                                             model_input.jsonBase64ImageROIs)
    # return {'task_id': str(task_id), 'status': 'Processing'}
    return TaskTicket(task_id=str(task_id), status='Processing')


@app.get('/AutomaticAppleSegmentationDetectron2Model/get_apple_automatic_rois_with_indicators_Detectron2_result/{task_id}',
         response_model=AutomaticAppleSegmentationWithIndicatorsOutput,
         status_code=200,
         responses={202: {'model': TaskTicket, 'description': 'Accepted: Not Ready'}})
async def get_AutomaticAppleSegmentationDetectron2Model_get_apple_automatic_rois_with_indicators_Detectron2_result(task_id):
    """Fetch result for given task_id"""
    task = AsyncResult(task_id)
    if not task.ready():
        return JSONResponse(status_code=202, content={'task_id': str(task_id), 'status': 'Processing'})
