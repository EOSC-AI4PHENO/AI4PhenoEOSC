...
def split_data(src_dir, val_pct, train_pct, test_pct):
    ...
    shutil.copy(os.path.join(src_dir, 'via_region_data.json'), dest_dirs[0])
    shutil.copy(os.path.join(src_dir, 'via_region_data.json'), dest_dirs[1])
    shutil.copy(os.path.join(src_dir, 'via_region_data.json'), dest_dirs[2])

    return dest_dirs

train_dir, val_dir, test_dir = split_data('Linden_Photos_Flowering_WellExposed', 0.7, 0.15, 0.15)
